...
def get_distinctions(person_result):
    ...
    SELECT DISTINCT ?distinction ?date ?distinctionLabel WHERE {{
        <{person_wikidata}> p:P166 [ ps:P166 ?distinction ; pq:P585 ?date ] .
        ?distinction  rdfs:label ?distinctionLabel . 
	    FILTER (lang(?distinctionLabel) = "fr")

    }}
    """
        sparql.setQuery(query)
        sparql.setReturnFormat(JSON)
        return [(r['distinction']['value'], r['date']['value'], r['distinctionLabel']['value']) for r in sparql.query().convert()['results']['bindings']]
    except:
        return []
...
PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>
PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>
SELECT DISTINCT ?person ?personWikidata ?personLabel ?distinction WHERE {
...
"""
    ...
    for person_result, distinctions in distinctions_people:
        for distinction in distinctions:
            dict_data.append({ 'person': person_result['person']['value'], 'personLabel': person_result['personLabel']['value'], 'distinction': distinction[0], 'date': distinction[1],'distinctionLabel': distinction[2]})

    with open('distinctions_people.csv', 'w') as csvfile:
        writer = csv.DictWriter(csvfile, fieldnames=['person', 'personLabel' , 'distinction', 'distinctionLabel', 'date'])
        writer.writeheader()
